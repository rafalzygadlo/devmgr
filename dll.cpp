...
//	m_CurrentCPAVerticesBufferPtr = NULL;
	...
	m_ShipCPA->SetRenderMode(GL_LINES);
	color.R = 255;color.G = 0;color.B = 0;color.A = 255;
	m_ShipCPA->SetColor(color);
	...
	m_FakeBow = new CObject();
	m_FakeBow->SetRenderMode(GL_LINES);
	color.R = 0;color.G = 255;color.B = 0;color.A = 255;
	m_FakeBow->SetColor(color);
	
	m_FakeStern = new CObject();
	m_FakeStern->SetRenderMode(GL_LINES);
	color.R = 255;color.G = 0;color.B = 0;color.A = 255;
	m_FakeStern->SetColor(color);

	m_ObjectArray.Append(m_Light0);
	m_ObjectArray.Append(m_Light1);
	m_ObjectArray.Append(m_Light2);
	m_ObjectArray.Append(m_ShipCPA);
	m_ObjectArray.Append(m_CPA);
	m_ObjectArray.Append(m_FakeShip);
	m_ObjectArray.Append(m_FakeBow);
	m_ObjectArray.Append(m_FakeStern);
	...
	m_Ticker1->Start(1000/GetControlFrequency());
...
CMapPlugin::~CMapPlugin()
{
	...
	for(size_t i = 0; i < m_ObjectArray.Length();i++)
	{
		CObject *ptr = m_ObjectArray.Get(i);
		ptr->ClearBuffers();
		delete ptr;
	}
...
void CMapPlugin::OnSetShip()
{
	...
	{
		double n1,n2;
		NewHDT(1000*1,&v); // w milisekundach
		NewLonLat(10*i,m_ShipStaticState[SHIP_LON],m_ShipStaticState[SHIP_LAT],m_ShipStaticState[SHIP_SOG],v,&n1,&n2);
		ptr.lon = n1;
		ptr.lat = n2;
	...
	m_FakeBow->SetCurrentPtr(true);
	m_FakeBow->ClearBuffers();
	m_FakeStern->SetCurrentPtr(true);
	m_FakeStern->ClearBuffers();

	nvPoint2d p11,p12,p21,p22;
	
	SAisData ptr1,ptr2;
	int idx = m_FakeShips.Length() - 1;
	ptr1 = m_FakeShips.Get(idx);
	ptr2 = m_FakeShips.Get(idx - 1);
	...
	FakeArrow(p11,p12,ptr1.hdg,ptr2.hdg,m_FakeBow);
	FakeArrow(p21,p22,ptr1.hdg+180,ptr2.hdg+180,m_FakeStern);
...
void CMapPlugin::FakeArrow(nvPoint2d p1, nvPoint2d p2, float hdg1, float hdg2, CObject *ptr)
{
	...
	//double out_x,out_y;
	//RotateZ(p1.x,p1.y,out_x,out_y,nvToRad(180));	p1.x = out_x;	p1.y = out_y;
	
	
	//double x = (distance) * cos(nvToRad(angle)) + p1.x;
	//double y = (distance) * sin(nvToRad(angle)) + p1.y;
	

	double x = p1.x + ((p1.x - p2.x)*10);
	double y = p1.y + ((p1.y - p2.y)*10);

	double sog = distance;
	
	ptr->AddPoint(p1);
	int id = ptr->GetVertexLength();
	ptr->AddIndice(id - 1);
...
void CMapPlugin::FakeShipBuffer(SAisData ptr)
{
	...
	p2.x =  0.0 * width;	p2.y =  0.6    * height;
	p3.x =  0.5 * width;	p3.y =  0.5    * height;
	p4.x =  0.5 * width;	p4.y = -0.3    * height;
	p5.x =  0.3 * width;	p5.y = -0.425  * height;
	p6.x =  0.0;			p6.y = -0.5    * height;
	p7.x = -0.3 * width;	p7.y = -0.425  * height;
	p8.x = -0.5 * width;	p8.y = -0.3    * height;
	...
	RotateZ(p8.x,p8.y,out_x,out_y,nvToRad(ptr.hdg));	p8.x = out_x;	p8.y = out_y;
#endif
	...
	m_FakeShip->AddPoint(p8);
	

	int id = m_FakeShip->GetVertexLength();

	m_FakeShip->AddIndice(id - 8);
	m_FakeShip->AddIndice(id - 7);
	m_FakeShip->AddIndice(id - 7);
	m_FakeShip->AddIndice(id - 6);
	m_FakeShip->AddIndice(id - 6);
	m_FakeShip->AddIndice(id - 5);
	m_FakeShip->AddIndice(id - 5);
	m_FakeShip->AddIndice(id - 4);
	m_FakeShip->AddIndice(id - 4);
	m_FakeShip->AddIndice(id - 3);
	m_FakeShip->AddIndice(id - 3);
	m_FakeShip->AddIndice(id - 2);
	m_FakeShip->AddIndice(id - 2);
	m_FakeShip->AddIndice(id - 1);
	m_FakeShip->AddIndice(id - 1);
	m_FakeShip->AddIndice(id - 8);
...
void CMapPlugin::SetShip(SFunctionData *data)
{
	...
	if(data->id_signal == SHIP_LAT)
		m_LATChanged = true;

	if(m_LONChanged && m_LATChanged)
...
bool CMapPlugin::NewPosition(int time)
{
	...
	double sogm = sog / 3600; 
	double dlatm = sogm * cos(ToRad(cog)) * sec;
	double dlonm = sogm * sin(ToRad(cog)) * sec;
	double lonDistance = nvDistance( lon, lat, lon + 1.0 , lat);
	double latDistance = nvDistance( lon, lat, lon , lat + 1.0);
		
	double nlon = lon + (dlonm / lonDistance);	// sta³a iloœæ km na 1 stopien
	double nlat = lat + (dlatm / latDistance);	// sta³a iloœæ km na 1 stopien
...
bool CMapPlugin::NewHDT(int time, double *va)
{
...
void CMapPlugin::PrepareCPAVerticesBuffer(nvPoint2d pt1,nvPoint2d pt2)
{
	...
	m_CPA->AddIndice(id - 2);
	m_CPA->AddIndice(id - 1);
...
void CMapPlugin::PrepareShipCPABuffer()
{
	...
	m_ShipCPA->SetCurrentPtr(true);
	m_ShipCPA->ClearBuffers();

	for(size_t i = 0; i < ais_get_ship_collision_count();i++)
	{
		PrepareShipCPAVerticesBuffer(ais_get_ship_collision_item(i));
	}

	m_ShipCPA->CopyBuffers();
	m_ShipCPA->SetCurrentPtr(false);
...
void CMapPlugin::PrepareShipCPAVerticesBuffer(SAisData *ptr)
{
	...
	m_Broker->Unproject(GetShipState(SHIP_LON),-GetShipState(SHIP_LAT),&to_x,&to_y);
	p2.x = to_x;
	p2.y = to_y;

	m_ShipCPA->AddPoint(p2);
	m_ShipCPA->AddPoint(p1);
	
				
	int id = m_ShipCPA->GetVertexLength();
	m_ShipCPA->AddIndice(id - 1);
	m_ShipCPA->AddIndice(id - 2);
...
void CMapPlugin::PrepareHDGVerticesBuffer(SAisData *ptr)
{
	...
	// od dziobu dlatego ten kod potrzebny
	p1 = m_HdtPoint;
